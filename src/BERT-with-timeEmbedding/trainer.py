...
from model import BERTLMTE, BERTTE
...
class BasicTrainer:
    def __init__(self, bert: BERTTE, epochs: int,
...
class BERTTrainerTE(BasicTrainer):
    def __init__(self, bert: BERTTE, vocab_size: int, epochs: int,
        ...
        super(BERTTrainerTE, self).__init__(bert=bert, epochs=epochs,
                                            tensorboard_log_dir=tensorboard_log_dir,
                                            output_path=output_path,
                                            train_dataloader=train_dataloader,
                                            with_cuda=with_cuda, log_freq=log_freq, save_steps=save_steps)

        self.model = BERTLMTE(bert, vocab_size).to(self.device)
    ...
    def iteration(self, epoch, data_loader):
        ...
        for i, data in data_iter:
            ...
            # print(type(data["token_input"]), type(data["time_input"]))
            # print(data["token_input"])
            # print(data["token_label"])
            # print(data["time_input"])
            # print(data["time_label"])
            mask_tp_output, mask_lm_output = self.model.forward(data["token_input"], data["time_input"])
            # print("mask_tp_output", mask_tp_output)
            # print("mask_lm_output", mask_lm_output)
