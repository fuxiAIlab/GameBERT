...
from model import BERTFull
from dataset import BERTDatasetFull
from trainer import BERTTrainerFull
...
params_str = "_{}_{}_{}_{}_{}_{}_{}_{}_{}_{}".format(max_seq_len, hidden, layers, attn_heads, expand_dim, stride,
                                                     lr, weight_decay, lambda_beta, warmup_steps)

tensorboard_log_dir = output_dir + '/tensorboard' + params_str


def construct_pos_mask():
...
def main():
    ...
    train_dataset = BERTDatasetFull(corpus_path, tokenizer, max_seq_len, corpus_lines=corpus_lines)
    batch_size = per_gpt_batch_size * n_gpu if n_gpu > 0 else 8
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size)

    bert = BERTFull(tokenizer.vocab_size, hidden=hidden, n_layers=layers, attn_heads=attn_heads, expand_dim=expand_dim)
    bert = bert.to(device)
    trainer = BERTTrainerFull(bert, tokenizer.vocab_size, epochs,
