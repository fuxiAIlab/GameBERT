...
vocab_file = os.path.join(base_dir, 'vocab')
...
def build_model(n_gpu, vocab):
    with tf.device('/cpu:0'):
        # Inputs
        model = Sequential()
        model.add(Dense(dense_size_1, input_dim=vocab.vocab_size, activation='relu',
...
def load_dataset(vocab):
    def __load_features(_dir: str, vocab: Vocab):
        samples = os.listdir(_dir)
        features = []
        for sample in tqdm.tqdm(samples):
            feature = [0] * vocab.vocab_size
            with open(os.path.join(_dir, sample), 'r', encoding='utf-8') as fin:
                ...
                for event in events:
                    ...
                    feature[vocab.token2idx(logdesignid)] += 1
