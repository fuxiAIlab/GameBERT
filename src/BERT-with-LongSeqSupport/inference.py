    ...
    for i in tqdm.tqdm(range(n_batches), desc=output_embds_file):
        ...
        with torch.no_grad():
            ...
            for j in range(n_batches_batches):
                ...
                outputs = model(torch.tensor(_input_ids).to(device), pos_mask)
...
def get_map_preload_embeddings(input_seq_dirs, output_embds_file):
    ...
    for i in tqdm.tqdm(range(n_batches), desc=output_embds_file):
        ...
        with torch.no_grad():
            outputs = model(torch.tensor(batch_input_ids).to(device), pos_mask)
            outputs = outputs.tolist()
...
if __name__ == "__main__":
    ...
    model_path = os.path.join("bert-with-longSequenceSupport", sys.argv[1])
    ...
    model = BERTLSSInference(transformer=transformer, sequence_summary_type='max_pooling').to(device)
